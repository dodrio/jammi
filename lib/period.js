...
function period (prizes, startTime, endTime, cb) {
  ...
  for (const prize of prizePool) {
    ...
    if (random > min && random <= max) {
      ...
      // when remaining prize equal to 1,
      // release the prize in latter half period.
      let seedrandomNum = seedrandom.alea(balance)()
      if (balance === 1 && seedrandomNum < 0.5) {
        seedrandomNum = 1 - seedrandomNum
      }

      const randomTimeOffset = Math.ceil(seedrandomNum * deltaTime)
